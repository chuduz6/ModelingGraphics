...
/**
 * File parser. File format described in the source file.
 * @author fabio
 */
...
public class FileFormat {
    ...
    private Object parseBuiltinArray(ParserNode n, Class c) throws ParserException {
        ...
        if(c.equals(Boolean.TYPE)) {
            // grab a bool array from the data
            boolean[] boolArray = parseBooleanArray(n.content);;
            ret = Array.newInstance(c,boolArray.length);
            for(int i = 0; i < boolArray.length; i ++) {
                Array.setBoolean(ret,i,boolArray[i]);
            }
        } else {
    ...
    private boolean[] parseBooleanArray(String content) throws ParserException {
        // grab all the token split by space
        StringTokenizer tokenizer = new StringTokenizer(content, " \n\r\t");
        
        int nElements = tokenizer.countTokens();
        boolean[] ret = new boolean[nElements];
        
        try {
            for (int i = 0; i < ret.length; i++) {
                ret[i] = Boolean.parseBoolean(tokenizer.nextToken());
            }
        } catch(Exception e) {
            throw new ParserException("Expected true/false",e);
        }
